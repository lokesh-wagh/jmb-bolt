...
app.post('/api/register', async (req, res) => {
  ...
  const mailOptions = {
    ...
    text: `Hello ${name},\n\nPlease verify your email by using this token:\n\n${token}\n\nOr visit: ${process.env.FRONTEND_VERIFY_URL || 'http://localhost:4000/api'}/verify?token=${token}\n\nThis link expires in 1 hour.`,
    html: `<p>Hello ${name},</p><p>Please verify your email by using this token:</p><pre>${token}</pre><p>Or click: <a href="${process.env.FRONTEND_VERIFY_URL || 'http://localhost:4000/api'}/verify?token=${token}">Verify email</a></p><p>This link expires in 1 hour.</p>`,
  };
